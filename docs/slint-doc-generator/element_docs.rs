...
fn extract_doc_comment(node: &parser::SyntaxNode) -> Option<String> {
    ...
    if let Some(parent) = node.parent() {
        if parent.kind() == SyntaxKind::ExportsList {
                        ...
                        text.strip_prefix("//! ").or_else(|| text.strip_prefix("//!"))
                    {
                        ...
                        if !content.starts_with('#') {
                            if let Some(last) = members.last_mut() {
                                if last.kind == MemberKind::SectionHeader {
...
fn write_sub_element(
    ...
    writeln!(file, "## {child_name} Sub-Element for `{parent_name}`")?;
    writeln!(file)?;
