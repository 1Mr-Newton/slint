...
pub struct RepeaterTracker<T: RepeatedItemTree> {
    ...
    #[pin]
    /// Toggled by `ensure_updated` when instances are actually added or
    /// removed. Layout and visit code register this as a dependency so
    /// they re-evaluate only after the update pass materializes the change,
    /// not when the model first becomes dirty.
    instance_generation: Property<bool>,
...
impl<C: RepeatedItemTree + 'static> Repeater<C> {
    ...
    pub fn ensure_updated(self: Pin<&Self>, init: impl Fn() -> ItemTreeRc<C>) -> bool {
        ...
        let changed = if self.data().project_ref().is_dirty.get() {
            ...
            let ig = self.data().project_ref().instance_generation;
            ig.set(!ig.get());
            true
        } else {
            false
    ...
    pub fn ensure_children_instantiated(&self) -> bool {
    ...
    /// Like [`Self::track_model_changes`] but also registers the viewport
...
impl<C: RepeatedItemTree + 'static> Conditional<C> {
    ...
    pub fn ensure_updated(self: Pin<&Self>, init: impl Fn() -> ItemTreeRc<C>) -> bool {
        ...
        if changed {
            let ig = self.project_ref().instance_generation;
            ig.set(!ig.get());
        }
